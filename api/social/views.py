...
class UsernameSocialAccountRetrieve(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        exists = SocialAccountUsername.objects.filter(platform=str.upper(platform), value=value).exists()
        if exists:
            return SocialAccountUsername.objects.filter(platform=str.upper(platform), value=value)
        else:
            return []
...
class EmailSocialAccountRetrieve(generics.ListAPIView):
    ...
    def get_queryset(self):
        platform = self.kwargs['platform']
        value = self.kwargs['value']
        exists = SocialAccountEmail.objects.filter(value=value, platform=str.upper(platform)).exists()
        if exists:
            return SocialAccountEmail.objects.filter(value=value, platform=str.upper(platform))
        else:
            return []
